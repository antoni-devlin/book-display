...
let fields = "fields=items(id,volumeInfo(title,authors,publishedDate,description,pageCount,imageLinks(thumbnail),infoLink)"; //The Google openbooks api lets you select only certain fields to return in the response, in a way that makes me think of GraphQL... That's what this is doing.
...
function parseBooks(booksObject) {
    booksObject["items"].forEach((book) => {
        ...
        console.log(`${bookId} - ${title} by ${author} (${datePublished}) - ${pageCount} pages long
      ${description}
      ${thumbnailPath}
